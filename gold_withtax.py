...
class TaxMan:
    ...
    def tax_agents(self):
        ...
        for agent in self.agents:
            amount = 0
            if (agent.wallet < 12000):
                # low income tax bracket
                amount = agent.wallet * (self.tax_rate * 0.75)
                agent.poor = True
                self.poor_count += 1
            elif (agent.wallet < 20000):
                # mid income tax bracket
                amount = agent.wallet * self.tax_rate
                agent.poor = False
            elif (agent.wallet >= 20000):
                #high income tax bracket
                amount = agent.wallet * (self.tax_rate * 0.60)
                agent.poor = False
            remainder = agent.deduct(amount)
            self.income += amount

    def refund_agents(self):
        slush_fund = self.income - (self.income * 0.2)
        slush_fund_per = 0
        if (self.poor_count == 0):
            slush_fund = 0
        else:
            slush_fund_per = slush_fund / self.poor_count
        fair = (self.income - slush_fund) / self.agent_count
        for agent in self.agents:
            agent.credit(fair)
            if (agent.poor):
                agent.credit(slush_fund_per)
                self.income -= slush_fund_per
            self.income -= fair
        self.reset_zero()
